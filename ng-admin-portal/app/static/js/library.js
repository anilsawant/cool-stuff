window.onload = function() {
  window.sessionStorage.setItem('faAdmin', JSON.stringify({"name": "ADMIN"}));
...
let init = function () {
  window.fa = {
    "user": JSON.parse(window.sessionStorage.getItem('faAdmin')),
    "data": {
      "currentSelection": {
        "type": "",
        "brand": "",
...
let initializeFirebase = function () {
  ...
  window.fa.firebaseDatabase = firebase.database().ref('videos/');
  window.fa.firebaseStorage = firebase.storage().ref('videos/');
  window.fa.firebaseDatabase.on('value', function (snapshot) {
    //snapshot.val() contains the root obj i.e. videos/ - it contains the device details tree
    window.fa.data.availableDevicesTree = snapshot.val();
    if (isInitializing) {
