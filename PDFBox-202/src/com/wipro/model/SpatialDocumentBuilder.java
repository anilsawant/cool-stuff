...
/**
 * This class creates a spatial representation of a PDF.
 * <p>
 * This class is a copy of PDFTextStripper.java written by Ben Litchfield. 
 * It also overrides processOperator() to populate 'imageLocations' Map
 * </p>
 * @author AN303936
 */
public class SpatialDocumentBuilder extends PDFTextStripper {
	...
	// Map<pageNumber,List<imageCoods[]>>; Co-ods as [x,y,width,height]
	private final Map<Integer, List<Float[]>> imageLocations = new HashMap<Integer, List<Float[]>>();
	...
	public Map<Integer, List<Float[]>> getImageLocations() {
	...
	protected void processOperator(Operator operator, List<COSBase> operands) throws IOException {
		...
		if ("Do".equals(operation)) {
			...
			if (xobject instanceof PDImageXObject) {
				...
				xywh[0] = ctmNew.getTranslateX();
				xywh[1] = getCurrentPage().getMediaBox().getHeight() - ctmNew.getTranslateY() - imageYScale;
				xywh[2] = imageXScale;
				xywh[3] = imageYScale;
				if (imageLocations.containsKey(getCurrentPageNo())) {
					List<Float[]> imageCoods = imageLocations.get(getCurrentPageNo());
					imageCoods.add(xywh);
				} else {
					List<Float[]> imageCoods = new ArrayList<Float[]>();
					imageCoods.add(xywh);
					imageLocations.put(getCurrentPageNo(), imageCoods);
				}
			...
			while (textIter.hasNext()) {
				...
				if (lastPosition != null) {
					...
					if (!overlap(positionY, positionHeight, maxYForLine, maxHeightForLine)) {
						createSpatialWordsWithSupTagged(line);
	...
	/**
	 * Create SpatialWords from a line
	 * @param line
	 */
	private void createSpatialWords( List<LineItem> line) {
			...
			if( textPosition != null ) {
				if( textPosition.getUnicode().equals(" ") || textPosition.getCharacterCodes()[0] == 32) {
					if( !textPosList.isEmpty() ) {
						...
						spatialWords.add(spatialWord);//this is temp. Remove it if word count is not required. 
	...
	/**
	 * Create words from Line with superscript/subscript characters as separate words
	 * @param line
	 */
	private void createSpatialWordsWithSupSubAsSeparateWords( List<LineItem> line) {
		List<SpatialWord> spatialWordsList = new ArrayList<SpatialWord>();
		List<TextPosition> textPosList = new ArrayList<TextPosition>();
		float prevWordY = 0.0f;
		for (int i = 0; i < line.size(); i++) {
			TextPosition textPosition = line.get(i).getTextPosition();
			if( textPosition != null ) {
				if ( textPosList.isEmpty() ) {
					prevWordY = textPosition.getY();
				}
				if( textPosition.getUnicode().equals(" ") || textPosition.getCharacterCodes()[0] == 32) {
					if( !textPosList.isEmpty() ) {
						SpatialWord spatialWord = new SpatialWord(textPosList);
				...
				//PDFStripper correctly identifies the y-coordinate of 
				//a superscript/subscript word as being lower/higher than the previous word.
				//But it attaches the superscript/subscript word to the previous word. 
				//This else if block is to identify that and create a new word for the superscript/subscript.
				else if( Math.abs(prevWordY - textPosition.getY()) > Constants.SUPERSCRIPT_THRESHOLD ){
					if( !textPosList.isEmpty() ) {
						SpatialWord spatialWord = new SpatialWord(textPosList);
						spatialWords.add(spatialWord);//this is temp. Remove it if word count is not required.
						spatialWordsList.add( spatialWord );
				...
				//If its the last character in the line, construct the word immediately 
				//without waiting for a space(" ") as word separator
				else if( (i+1) == line.size() ){
					textPosList.add( textPosition );
					if( !textPosList.isEmpty() ) {
						SpatialWord spatialWord = new SpatialWord(textPosList);
	 ...
	 * @param line
	 * <p>
	 * Doesn't handle hanging superscripts. i.e. superscripts not attached to previous word
	 * </p>
	 */
	private void createSpatialWordsWithSupTagged( List<LineItem> line) {
		List<SpatialWord> spatialWordsList = new ArrayList<SpatialWord>();
		List<TextPosition> textPosList = new ArrayList<TextPosition>();
		float prevCharY = 0.0f;
		boolean isSuperScript = false;
		for (int i = 0; i < line.size(); i++) {
			TextPosition textPosition = line.get(i).getTextPosition();
			if( textPosition != null ) {
				if ( textPosList.isEmpty() ) {
					prevCharY = textPosition.getY();
				}
				if( textPosition.getUnicode().equals(" ") || textPosition.getCharacterCodes()[0] == 32) {
					if( !textPosList.isEmpty() ) {
						SpatialWord spatialWord = null;
						if (isSuperScript) {
							spatialWord = new SpatialWord(textPosList, true);
						}else {
							spatialWord = new SpatialWord(textPosList);
						}
						isSuperScript = false;
						spatialWords.add(spatialWord);//this is temp. Remove it if word count is not required. 
						spatialWordsList.add( spatialWord );
				...
				//But it attaches the superscript/subscript word to the previous word. 
				//This else if block is to identify that and create a new word for the superscript/subscript.
				else if( prevCharY - textPosition.getY() > Constants.SUPERSCRIPT_THRESHOLD){
					isSuperScript = true;
					if( !textPosList.isEmpty() ) {
						SpatialWord spatialWord = new SpatialWord(textPosList);
						spatialWords.add(spatialWord);//this is temp. Remove it if word count is not required.
	...
	protected void writePageEnd() throws IOException {
		...
		tempSpatialPage = new SpatialPage( tempSpatialArticlesPerPage, getCurrentPageNo(), currentPage.getMediaBox().getHeight(), currentPage.getMediaBox().getWidth() );// create a new SpatialPage
		tempSpatialPage.setAnnotations( currentPage.getAnnotations() );
		if ( imageLocations.containsKey(getCurrentPageNo()) ) {
			tempSpatialPage.setEmbeddedImageRectangles( new ArrayList<Float[]>(imageLocations.get(getCurrentPageNo())) );
		}
